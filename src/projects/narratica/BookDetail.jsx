    ...
    return (
      <div className="p-4 animate-pulse">
        <div className="flex flex-col md:flex-row items-center md:items-start gap-8 mb-10">
          <div className="w-48 h-48 rounded-lg shadow-lg bg-narratica-gray-light flex-shrink-0"></div>
          <div className="flex-1 space-y-4 mt-4 md:mt-0">
            <div className="h-8 bg-narratica-gray-light rounded w-3/4"></div>
            <div className="h-6 bg-narratica-gray-light rounded w-1/2"></div>
            <div className="h-4 bg-narratica-gray-light rounded w-full mt-6"></div>
          </div>
        </div>
      </div>
    );
      ...
      <div className="flex flex-col md:flex-row items-center md:items-start gap-8 mb-10">
        <section className="w-48 h-48 rounded-lg shadow-lg flex-shrink-0">
          {secureImageUrl ? (
            ...
            <div className="w-full h-full flex items-center justify-center bg-narratica-gray-light rounded-lg">
              <svg className="w-12 h-12 text-narratica-text-secondary" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1.5">
        ...
        <div>
          ...
          <div className="text-lg text-narratica-text-secondary mt-2">
            by{' '}
            {bookDetails.authors.map((author, index) => (
              <React.Fragment key={index}>
                <button
                  onClick={() => onAuthorSearch(`${author.first_name} ${author.last_name}`)}
                  className="hover:text-narratica-text-primary hover:underline focus:outline-none focus:underline"
                >
          ...
          <button onClick={toggleFavorite} title="Add to Favorites" className="text-narratica-green hover:text-narratica-text-primary transition-colors mt-4">
            {isFavorited ? (
          ...
          <p className="text-narratica-text-secondary mt-4 text-sm max-w-prose">
      ...
      <div>
        ...
        <ul className="space-y-2">
          {tracks?.map((track, index) => (
            <li
              key={track.id}
              className="flex items-center justify-between p-3 bg-narratica-gray-dark hover:bg-narratica-gray-light rounded-lg cursor-pointer transition"
              onClick={() => handlePlay(track, index)}
            >
              <div className="flex items-center gap-4 overflow-hidden">
                <span className="text-narratica-text-secondary">{track.section_number}.</span>
                <p className="font-medium truncate">{track.title}</p>
              </div>
              <PlayIcon className="h-6 w-6 text-narratica-text-secondary flex-shrink-0" />
            </li>
          ))}
        </ul>
      </div>
